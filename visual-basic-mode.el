...
(defcustom visual-basic-untabify-on-save nil
  "If non-nil, remove tabs when saving file."
  :group 'visual-basic
  :type 'boolean)
...
(defconst visual-basic-continuation-regexp
  (concat "^\\(.*\\([^_[:alnum:]]\\|[^[:alpha:]_][0-9]+\\)\\)?"
          visual-basic-looked-at-continuation-regexp))
  ...
  (let* ((start-point (point))
    ...
    (while (null line-end)
	  ...
	  (if (visual-basic-at-line-continuation)
	    ...
	    (setq complete-line
		  (cons
                   (buffer-substring-no-properties line-beg line-end) complete-line)))
	;; not a folded line
	(setq complete-line
...
be folded over several code lines."
  (if (looking-at visual-basic-if-regexp)
	...
	(let (p1
	  ...
	  ;; remove any VB string from complete line, as strings may disrupt :
          ;; and ' detection
...
(defun visual-basic-find-matching-if ()
  "Set pointer on the line with If stating the If ... Then ... [Else/Elseif ...] ... 
End If block containing pointer."
  (visual-basic-find-predicate-matching-stmt
   'visual-basic-if-not-on-single-line
   (lambda ()
...
(defun visual-basic-find-matching-while ()
  "Set pointer on the line with While stating the `While ... Wend' block 
containing pointer."
  (visual-basic-find-matching-stmt visual-basic-while-regexp
                                   visual-basic-wend-regexp))

(defun visual-basic-find-matching-with ()
  "Set pointer on the line with With stating the `With ... End with' block 
containing pointer."
...
(defun visual-basic-find-matching-begin ()
  "Set pointer on the line with Begin stating the `Begin ... End' 
block containing pointer."
  (let ((original-point (point)))
    (visual-basic-find-matching-stmt visual-basic-begin-regexp
                                     visual-basic-end-begin-regexp)
    (if (bobp)
        ;; failed to find a matching begin so assume that it is
...
(defun visual-basic--make-keyword-re (keyword)
  ...
  (format "[%c%c]%s" (aref keyword 0)
          (logxor (aref keyword 0) 32) (substring keyword 1)))
...
(defun visual-basic-beginning-of-defun ()
  "Set the pointer at the beginning of the Sub/Function/Property within which the 
pointer is located."
  (interactive)
...
(defun visual-basic-mark-defun ()
  "Set the region pointer around Sub/Function/Property within which the pointer is 
located."
  (interactive)
  (beginning-of-line)
...
(defun visual-basic-indent-defun ()
  "Indent the function within which the pointer is located.  
This has a border effect on mark."
  ...
  (let (end-statement end-indent)
	...
	(while
		...
		nil)
	       ((looking-at visual-basic-case-regexp)
		(setq  end-statement  "End Select"
		       end-indent (max 0 (- (current-indentation)
                                            visual-basic-mode-indent)))
		nil)
	       ((looking-at visual-basic-begin-regexp)
		  ...
		  (cond
		   ...
		   ((eq vb-idom 'vba)
		    (when (looking-at "\\s-+\\(Each\\s-+\\|\\)\\([^ \t\n\r]+\\)")
		      (setq end-statement
                            (concat end-statement " " (match-string 2)))))))
		nil)
...
(defun visual-basic-keyword-upper-lower (lst)
  "Make completion list."
  (append
   (sort lst 'string<)
   (sort (mapcar 'upcase lst) 'string<)
   (sort (mapcar 'downcase lst) 'string<)))
...
;;* Menu
(defvar visual-basic-menu
  '("VBS"
    ["Close Block" visual-basic-close-block :keys "C-c ]"]
    ["Split Line" visual-basic-split-line :keys "M-C-j"]
    ["Fill/Indent" visual-basic-fill-or-indent :keys "M-q"]
    "--"
    ["Start REPL" vbs-repl :keys "C-c C-z"]
    ["Start IDE" visual-basic-start-ide :keys "C-c C-c"]))

;;* Map
(defvar visual-basic-mode-map
  (let ((map (make-sparse-keymap)))
    (easy-menu-define nil map nil visual-basic-menu)
    (define-key map (kbd "TAB")             #'visual-basic-indent-line)
    (define-key map (kbd "RET")             #'visual-basic-newline-and-indent)
    (define-key map (kbd "M-C-a")           #'visual-basic-beginning-of-defun)
    (define-key map (kbd "M-C-e")           #'visual-basic-end-of-defun)
    (define-key map (kbd "M-C-h")           #'visual-basic-mark-defun)
    (define-key map (kbd "M-C-\\")          #'visual-basic-indent-region)
    (define-key map (kbd "M-q")             #'visual-basic-fill-or-indent)
    (define-key map (kbd "M-C-j")           #'visual-basic-split-line)
    (define-key map (kbd "C-c ]")           #'visual-basic-close-block)
    (cond (visual-basic-winemacs-p
           (define-key map (kbd "C-c C-c")  #'visual-basic-start-ide))
          (visual-basic-win32-p
           (define-key map (kbd "C-c C-c")  #'visual-basic-start-ide)))
    (if visual-basic-xemacs-p
        (progn
          (define-key map "\M-G"            #'visual-basic-grep)
          (define-key map '(meta backspace) #'backward-kill-word)
          (define-key map '(control meta /) #'visual-basic-new-sub)))
    map))

;;;###autoload
(define-derived-mode visual-basic-mode prog-mode "VBS"
...
\\{visual-basic-mode-map}"
  ...
  (setq-local indent-line-function 'visual-basic-indent-line)

  (if visual-basic-fontify-p
      (visual-basic-enable-font-lock))

  ;; imenu
  ;; These are automatically buffer local
  ...
  ;; company-keyword completions
  (when (featurep 'company)
    (require 'company-keywords)
    (make-local-variable 'company-keywords-alist)
    (push (append '(visual-basic-mode)
                  (visual-basic-keyword-upper-lower visual-basic-all-keywords))
          company-keywords-alist))
  
  ;; This should be the users choice
  (when visual-basic-untabify-on-save
    (add-hook 'local-write-file-hooks 'visual-basic-untabify nil t))
...
;;;###autoload
(add-to-list 'auto-mode-alist '("\\.\\(bas\\|frm\\|cls\\)$" . visual-basic-mode))
